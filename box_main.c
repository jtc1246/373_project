...
//	  HAL_Delay(500);
    ...
    /* USER CODE BEGIN 3 */
    int s = HAL_UART_Receive(UP, &data, 1, 1);
    if(s == HAL_OK) {
      uint8_t addr = (data >> 2) & 0x3F;
      uint8_t command = data & 0x03;
      if(command == 3) { // ping stream
        if(addr == 0){
          data = 3;
          ...
          int s2 = HAL_UART_Receive(DOWN, &data, 1, (addr + 1) * TIMEOUT_PER_BOX);
          uint8_t new_command = 3;
          uint8_t new_addr  = 0;
          if(s2 == HAL_OK) {
            new_addr = (data >> 2) & 0x3F;
            new_addr = new_addr + 1;
          }
          data = (new_addr << 2) | new_command;
          HAL_UART_Transmit(UP, &data, 1, 1000);
        }
      }
      else if(addr != 0) {
        ...
        if(status != STATUS_CLOSED){
          ...
          *tim4_ccr2 &= ~CCR_MASK;
          *tim4_ccr2 |= 75;
          HAL_GPIO_WritePin(GPIOB ,GPIO_PIN_7, 0); // LED
        }
      }
        ...
        if(status != STATUS_OPEN){
          ...
          *tim4_ccr2 &= ~CCR_MASK;
          *tim4_ccr2 |= 180;
          HAL_GPIO_WritePin(GPIOB ,GPIO_PIN_7, 1); // LED
        }
      }
      else if(command == 2) { // ping state
        data = 1 - status;
        HAL_UART_Transmit(UP, &data, 1, 1000);
      }
    }
